...
final class BillingHandlerUnitTests: TestCase {
    ...
    private func runRouteChangeTest(initialRouteWaypoints: [CLLocationCoordinate2D],
        ...
        func generateRoute(from legs: [RouteLeg], waypoints: [CLLocationCoordinate2D]) -> (Route, RouteOptions) {
        ...
        let (initialRoute, initialRouteOptions) = generateRoute(from: initialRouteLegs, waypoints: initialRouteWaypoints)
        let newRouteSteps = generateSteps(from: newRouteWaypoints)
        let newRouteLegs = generateLegs(from: newRouteSteps)
        let (newRoute, newRouteOptions) = generateRoute(from: newRouteLegs, waypoints: newRouteWaypoints)

        let dataSource = DataSource()
        let routeController = RouteController(along: initialRoute,
                                              routeIndex: 0,
                                              options: initialRouteOptions,
                                              dataSource: dataSource)

        routeController.updateRoute(with: (newRoute, 0), routeOptions: newRouteOptions)
